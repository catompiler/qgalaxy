...
bool SpiralGalaxy::generate_old()
{
    ...
    const size_t stars_count = m_stars_count - 1;
    const qreal average_mass = (m_star_mass_min + m_star_mass_max) * 0.5f / stars_count;

    const qreal eccentricity = 0.35;

    const qreal angle_max = 1.5 * M_PI;
    const qreal min_delta_phi = 0.03125 * angle_max;
    const qreal max_delta_phi = 0.45 * angle_max;
    ...
    for(size_t i = 1; i < m_stars_count; i ++){
        ...
        r = min_radius + (/**/1.0 - sqrt/**/(getRanduf())) * avail_dradius;

        max_h = depth_div_radius * m_radius * pow(1.0 - r/m_radius, 2);
        h = max_h * getRandsf();

        //phi = getRanduf() * _2pi;
        phi = lerp(static_cast<qreal>(0.0), static_cast<qreal>(angle_max),
                    (r - min_radius) / avail_dradius);
        phi += rand()%2 ? M_PI : 0.0;
        phi += (1.0 - sqrt(getRanduf()) * 2.0) * lerp(min_delta_phi, max_delta_phi, 1.0 - r / m_radius);
        ...
        ellipse_a = r;
        ellipse_b = ellipse_a * sqrt(1.0 - eccentricity * eccentricity);

        //mass_stars_in_radius = average_mass * (stars_count - 1) * pow(r / _radius, 2.0);
        mass_stars_in_radius = average_mass * (stars_count - 1) *
                ((M_PI * ellipse_a * ellipse_b) / (M_PI * m_radius * m_radius));
        sum_mass = mass_stars_in_radius + m_black_hole_mass;

        v = sqrt(G * sum_mass / r * (1.0 - eccentricity));

        vel = QVector3D::crossProduct(galaxy_up, QVector3D(pos - bh_pos).normalized()) * v;
...
bool SpiralGalaxy::generate()
{
    ...
    const size_t spirals_count = rand() % 4 + 2;
    ...
    const qreal ellipse_eccentricity = getRanduf() * 0.3 + 0.2;

    const qreal _2pi = 2.0 * M_PI;
    const qreal angle_max = _2pi + M_PI * getRandsf();
    const qreal angle_delta = 0.25 * M_PI / spirals_count;
